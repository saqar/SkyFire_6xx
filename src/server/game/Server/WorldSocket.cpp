...
#define SHA_DIGEST_SIZE 20
    ...
    if (fields[3].GetUInt8() == 1) // if ip is locked
    {
        if (strcmp(fields[2].GetCString(), address.c_str()))
        {
            ...
            sScriptMgr->OnFailedAccountLogin(loginServerId);
            DelayedCloseSocket();
            return;
        }
    }
    ...
    sScriptMgr->OnAccountLogin(loginServerId);

    _worldSession = new WorldSession(loginServerId, battlenetAccountId, shared_from_this(), AccountTypes(security), expansion, mutetime, locale, recruiter, isRecruiter);
