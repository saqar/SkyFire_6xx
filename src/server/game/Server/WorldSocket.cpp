...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    recvPacket.read_skip<uint32>(); // loginServerId (signed) 
    recvPacket >> clientBuild;
    recvPacket.read_skip<uint32>(); // region
    recvPacket.read_skip<uint32>(); // siteId
    recvPacket.read_skip<uint32>(); // realmId
    recvPacket.read_skip<uint8>();  // loginServerType
    recvPacket.read_skip<uint8>();  // buildType
    ...
    recvPacket.ReadBits(1); // Use IPv6 (you can't even use IPv6, lmao)

    recvPacket >> addonSize;
    addonsData.resize(addonSize); // Addon stuff
