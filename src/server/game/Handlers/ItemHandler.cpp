...
void WorldSession::HandleSplitItemOpcode(WorldPacket& recvData)
{
    ...
    uint32 invItemCount = recvData.ReadBits(2);
    for (uint32 i = 0; i < invItemCount; i++)
    {
        recvData.read_skip<uint8>();    // ContainerSlot
...
void WorldSession::HandleBuyItemOpcode(WorldPacket& recvData)
{
    ...
    recvData.read_skip<uint32>();
    recvData.read_skip<uint8>(); // 2x Bit
    recvData >> count >> slot >> bagSlot;
...
void WorldSession::HandleAutoBankItemOpcode(WorldPacket& recvPacket)
{
    ...
    // Currently not implemented
    uint32 invItemCount = recvPacket.ReadBits(2);
    for (uint32 i = 0; i < invItemCount; i++)
    {
...
void WorldSession::HandleAutoStoreBankItemOpcode(WorldPacket& recvPacket)
{
    ...
    for (uint32 i = 0; i < invItemCount; i++)
    {
        recvPacket.read_skip<uint8>();    // ContainerSlot
        recvPacket.read_skip<uint8>();    // Slot
    }
