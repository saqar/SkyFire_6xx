...
{
    public:
        PlayerTextBuilder(WorldObject* obj, uint8 gender, WorldObject* speaker, ChatMsg msgtype, uint8 textGroup, uint32 id, uint32 language, WorldObject const* target)
            : _source(obj), _gender(gender), _talker(speaker), _msgType(msgtype), _textGroup(textGroup), _textId(id), _language(language), _target(target)
        {
...
uint32 CreatureTextMgr::SendChat(Creature* source, uint8 textGroup,  WorldObject const* whisperTarget /*= NULL*/, ChatMsg msgType /*= CHAT_MSG_ADDON*/, Language language /*= LANG_ADDON*/, CreatureTextRange range /*= TEXT_RANGE_NORMAL*/, uint32 sound /*= 0*/, Team team /*= TEAM_OTHER*/, bool gmOnly /*= false*/, Player* srcPlr /*= NULL*/)
{
    ...
    if (srcPlr)
    {
        PlayerTextBuilder builder(source, finalSource->getGender(), finalSource, finalType, iter->group, iter->id, finalLang, whisperTarget);
...
void CreatureTextMgr::SendSound(Creature* source, uint32 sound, ChatMsg msgType, WorldObject const* whisperTarget, CreatureTextRange range, Team team, bool gmOnly)
{
    ...
    data << uint32(sound);
    data << ObjectGuid(source->GetGUID128());
...
std::string CreatureTextMgr::GetLocalizedChatString(uint32 entry, uint8 gender, uint8 textGroup, uint32 id, LocaleConstant locale) const
{
    ...
    std::string baseText = "";
    BroadcastText const* broadcastText = sObjectMgr->GetBroadcastText(groupItr->BroadcastTextId);

    if (broadcastText)
        baseText = broadcastText->GetText(locale, gender);
    else
        baseText = groupItr->text;

    if (locale != DEFAULT_LOCALE && !broadcastText)
    {
