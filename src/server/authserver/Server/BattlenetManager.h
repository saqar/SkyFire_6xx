...
public:
    ...
    bool HasProgram(std::string const& program) const
    {
    #ifdef IGNORE_WOW_VERSION
        return true;
    #else
        return _programs.count(program) != 0;
    #endif
    }
    bool HasPlatform(std::string const& platform)
    {
    #ifdef IGNORE_WOW_VERSION
        return true;
    #else
        return _platforms.count(platform) != 0;
    #endif
    }
